...
public static class SecureLogging
{
    ...
    public static object CreateSafeHttpContext(HttpContext? httpContext)
    {
        ...
        return new
        {
            ...
            ClientIP = SanitizeForLog(httpContext.Connection.RemoteIpAddress?.ToString(), maxLength: 45, placeholder: "[unknown-ip]"),
            UserAgent = SanitizeForLog(request.Headers["User-Agent"].ToString(), maxLength: 100, placeholder: "[no-user-agent]")
        };
    }
}
