...
using _Imports = MapMe.Client._Imports;
using AuthenticationService = MapMe.Services.AuthenticationService;
using MapMeAuth = MapMe.Services.IAuthenticationService;
using MessageSummary = MapMe.DTOs.MessageSummary;
using UserSummary = MapMe.DTOs.UserSummary;
...
// Data access: prefer Cosmos if configured; else, in-memory fallback
    ...
    var cosmosDatabase = builder.Configuration["CosmosDb:DatabaseName"] ?? "mapme";
...
// Validate Cosmos DB configuration
    var useCosmos = !string.IsNullOrWhiteSpace(cosmosEndpoint) &&
                    ...
                    !cosmosEndpoint.Contains("YOUR_COSMOS_DB_ENDPOINT_HERE") &&
                    !cosmosKey.Contains("YOUR_COSMOS_DB_PRIMARY_KEY_HERE");

// Production environment MUST have Cosmos DB configured
    if (builder.Environment.IsProduction() && !useCosmos)
    {
        var errorMessage = "❌ PRODUCTION ERROR: Cosmos DB configuration is required in production environment.\n" +
                           "Please configure CosmosDb:Endpoint and CosmosDb:Key in your production settings.\n" +
                           "Application startup aborted for security reasons.";
        Console.WriteLine(errorMessage);
        throw new InvalidOperationException("Cosmos DB configuration is required in production environment. " +
                                            "Configure CosmosDb:Endpoint and CosmosDb:Key in production settings.");
    }

// Log repository selection for debugging
    if (useCosmos)
    {
        Console.WriteLine($"✅ Using Cosmos DB: {cosmosEndpoint} (Database: {cosmosDatabase})");
    }
    else
    {
        Console.WriteLine("⚠️  Using in-memory repositories (Cosmos DB not configured or using placeholders)");
        if (builder.Environment.IsProduction())
        {
            Console.WriteLine("   ❌ This is not allowed in production environment!");
        }

        if (!string.IsNullOrWhiteSpace(cosmosEndpoint))
        {
            Console.WriteLine($"   Cosmos endpoint found: {cosmosEndpoint}");
        }

        if (!string.IsNullOrWhiteSpace(cosmosKey))
        {
            Console.WriteLine(
                $"   Cosmos key found: {(cosmosKey.Length > 10 ? cosmosKey.Substring(0, 10) + "..." : "short key")}");
        }
    }
...
// Note: Session repository no longer needed for JWT authentication
    ...
    builder.Services.AddScoped<MapMeAuth, AuthenticationService>();
...
// Authentication API Endpoints
    ...
    app.MapGet("/api/auth/validate-token",
        async (HttpContext context, MapMeAuth authService, ILogger<Program> logger) =>
        {
            ...
            logger.LogInformation("[DEBUG] User validation result: {UserFound}",
                user != null ? $"Found user {SecureLogging.SanitizeForLog(user.Username)}" : "No user found");
    ...
    app.MapPost("/api/auth/change-password",
        async (ChangePasswordRequest request, HttpContext context, MapMeAuth authService) =>
        {
            var token = GetJwtTokenFromRequest(context);
            if (string.IsNullOrEmpty(token)) return Results.Unauthorized();
...
// Profiles API
    app.MapPost("/api/profiles",
        async (CreateProfileRequest req, IUserProfileRepository repo, HttpContext context, MapMeAuth authService) =>
        {
    ...
    app.MapGet("/api/profiles/{id}",
        async (string id, IUserProfileRepository repo, HttpContext context, MapMeAuth authService) =>
        {
            var currentUserId = await GetCurrentUserIdAsync(context, authService);
            if (currentUserId == null)
                return Results.Unauthorized();
...
// Add endpoint for current user (for JavaScript compatibility)
    app.MapGet("/api/users/current_user",
        async (HttpContext context, IUserProfileRepository repo, MapMeAuth authService) =>
        {
            var currentUserId = await GetCurrentUserIdAsync(context, authService);
            if (currentUserId == null)
                return Results.Unauthorized();
    ...
    app.MapGet("/api/users/{userId}",
        async (string userId, IUserProfileRepository repo, HttpContext context, MapMeAuth authService) =>
        {
            var currentUserId = await GetCurrentUserIdAsync(context, authService);
            if (currentUserId == null)
                return Results.Unauthorized();
...
// DateMarks API
    app.MapPost("/api/datemarks", async (UpsertDateMarkRequest req, IDateMarkByUserRepository repo, HttpContext context,
...
// Chat API Endpoints
    app.MapPost("/api/chat/messages", async (SendMessageRequest req, IChatMessageRepository messageRepo,
        IConversationRepository conversationRepo, IUserProfileRepository userRepo, HttpContext context,
        MapMeAuth authService) =>
    {
    ...
    app.MapGet("/api/chat/conversations", async (IConversationRepository conversationRepo,
        IUserProfileRepository userRepo, HttpContext context, MapMeAuth authService) =>
    {
        ...
        await foreach (var conversation in conversationRepo.GetByUserAsync(userId))
        {
            ...
            var conversationResponse = new ConversationResponse(
                ...
                LastMessage: conversation.LastMessageContent != null
                    ? new MessageSummary(
    ...
    app.MapPost("/api/chat/messages/read", async (MarkAsReadRequest req, IChatMessageRepository messageRepo,
        IConversationRepository conversationRepo, HttpContext context, MapMeAuth authService) =>
    {
        var userId = await GetCurrentUserIdAsync(context, authService);
        if (userId == null)
            return Results.Unauthorized();
    ...
    app.MapPost("/api/chat/conversations/archive", async (ArchiveConversationRequest req,
        IConversationRepository conversationRepo, HttpContext context, MapMeAuth authService) =>
    {
        var userId = await GetCurrentUserIdAsync(context, authService);
        if (userId == null)
    ...
    app.MapDelete("/api/chat/messages/{messageId}", async (string messageId, IChatMessageRepository messageRepo,
        HttpContext context, MapMeAuth authService) =>
    {
        var userId = await GetCurrentUserIdAsync(context, authService);
        if (userId == null)
            return Results.Unauthorized();
    ...
    app.MapGet("/api/chat/messages/new", async (IChatMessageRepository messageRepo,
        IConversationRepository conversationRepo, HttpContext context, MapMeAuth authService) =>
    {
        var userId = await GetCurrentUserIdAsync(context, authService);
        if (userId == null)
