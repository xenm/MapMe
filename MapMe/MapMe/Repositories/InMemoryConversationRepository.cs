...
public sealed class InMemoryConversationRepository : IConversationRepository
{
    ...
    public Task UpsertAsync(Conversation conversation, CancellationToken ct = default)
    {
        ...
        // Add to user indexes
        var user1Conversations = _conversationsByUser.GetOrAdd(conversation.Participant1Id, _ => new ConcurrentBag<string>());
        var user2Conversations = _conversationsByUser.GetOrAdd(conversation.Participant2Id, _ => new ConcurrentBag<string>());
    ...
    public async Task<Conversation> GetOrCreateConversationAsync(string userId1, string userId2, CancellationToken ct = default)
    {
    ...
    public Task SetArchiveStatusAsync(string conversationId, string userId, bool isArchived, CancellationToken ct = default)
    {
        if (_conversations.TryGetValue(conversationId, out var conversation))
        {
    ...
    public Task UpdateUnreadCountAsync(string conversationId, string userId, int unreadCount, CancellationToken ct = default)
    {
        if (_conversations.TryGetValue(conversationId, out var conversation))
        {
            var updatedConversation = userId == conversation.Participant1Id
