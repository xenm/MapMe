...
public class JwtApiIntegrationCorrectedTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Register_ValidUser_ReturnsSuccessWithJwtToken()
    {
        ...
        var authResponse = JsonSerializer.Deserialize<AuthenticationResponse>(responseContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
    ...
    public async Task ValidateToken_ValidToken_ReturnsUserData()
    {
        ...
        var responseContent = await response.Content.ReadAsStringAsync();
        var user = JsonSerializer.Deserialize<AuthenticatedUser>(responseContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
    ...
    public async Task CompleteAuthFlow_RegisterLoginValidate_AllSucceed()
    {
        ...
        var registerAuthResponse = JsonSerializer.Deserialize<AuthenticationResponse>(registerResponseContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        ...
        var loginAuthResponse = JsonSerializer.Deserialize<AuthenticationResponse>(loginResponseContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        ...
        var user = JsonSerializer.Deserialize<AuthenticatedUser>(validateResponseContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
    ...
    public async Task AuthenticationFlow_Performance_CompletesQuickly()
    {
        ...
        // Assert
        stopwatch.Stop();
        Assert.True(stopwatch.ElapsedMilliseconds < 5000, $"Authentication flow took {stopwatch.ElapsedMilliseconds}ms, expected < 5000ms");
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    }
