...
public class JwtSecurityCorrectedTests
{
    ...
    public void ValidateToken_TamperedPayload_ReturnsNull()
    {
        ...
        var parts = tokenResult.token.Split('.');
        var tamperedPayload = Convert.ToBase64String(Encoding.UTF8.GetBytes("{\"userId\":\"hacker\",\"username\":\"admin\"}"))
            .TrimEnd('=').Replace('+', '-').Replace('/', '_');
    ...
    public void ValidateToken_NoneAlgorithm_ReturnsNull()
    {
        ...
        var payload = $"{{\"userId\":\"{Guid.NewGuid()}\",\"username\":\"hacker\",\"email\":\"hacker@example.com\",\"exp\":{DateTimeOffset.UtcNow.AddHours(1).ToUnixTimeSeconds()}}}";
