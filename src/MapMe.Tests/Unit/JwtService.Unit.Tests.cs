...
public class JwtServiceCorrectedTests
{
    ...
    public void RefreshToken_ValidTokenNearExpiry_ReturnsNewToken()
    {
        ...
        var refreshResult =
            customJwtService.RefreshTokenWithCustomWindow(tokenResult.token, user, TimeSpan.FromHours(25));
    ...
    public void GenerateToken_Performance_CompletesQuickly()
    {
        ...
        Assert.True(stopwatch.ElapsedMilliseconds < 1000,
            $"100 token generations took {stopwatch.ElapsedMilliseconds}ms, expected < 1000ms");
    }
    ...
    public void ValidateToken_Performance_CompletesQuickly()
    {
        ...
        stopwatch.Stop();
        Assert.True(stopwatch.ElapsedMilliseconds < 1000,
            $"100 token validations took {stopwatch.ElapsedMilliseconds}ms, expected < 1000ms");
    }
...
public class TestableJwtService : JwtService
{
    ...
    public (string token, DateTimeOffset expiresAt)? RefreshTokenWithCustomWindow(string token, User user,
        TimeSpan customRefreshWindow)
    {
        try
        {
