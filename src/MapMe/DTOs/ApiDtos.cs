...
)
{
    public DateMark ToDateMark(DateTimeOffset now)
    {
        ...
        var tileKey = Geo.TileKey(Latitude, Longitude, precision: 3);
        var types = PlaceTypes ?? Array.Empty<string>();
        var place = PlaceName is null && PlaceId is null && types.Count == 0 && PlaceRating is null &&
                    PlacePriceLevel is null
            ? null
