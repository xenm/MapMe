...
# Run from the repo root or src folder; normalize to src folder for test path stability
...
DEFAULT_RESULTS_DIR="$REPO_ROOT/TestResults/Unit/$TIMESTAMP"
TEST_RESULTS_DIR=${TEST_RESULTS_DIR:-$DEFAULT_RESULTS_DIR}
mkdir -p "$TEST_RESULTS_DIR"

run_tests() {
  echo "Running Unit tests (client-side logic and business rules)..."
  ...
  echo "Results (.trx): $TEST_RESULTS_DIR/Unit.trx"
